...
import org.Indproc.util.StringMD;
...
public class LoginBean extends Utils {
	 ...
	 * **/
	public void login() throws NamingException,  SQLException, IOException {
		...
		if (rows == 0) {
			msj = new FacesMessage(FacesMessage.SEVERITY_ERROR, "El usuario no existe", "");
		...
		//Valida que usuario y claves sean los mismos, realiza el login y crea la variable de session !md.getStringMessageDigest(clave,StringMD.SHA256).equals(vLclave)
		if(usuario.equals(vLusuario) && !md.getStringMessageDigest(clave,StringMD.SHA256).equals(vLclave)){
			...
			msj = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Clave inválida", "");
			FacesContext.getCurrentInstance().addMessage(null, msj);
			...
			FacesContext.getCurrentInstance().getExternalContext().redirect("/indproc/jsf/index.xhtml"); 
    ...
    public void logout() throws IOException{
    	...
    	FacesContext.getCurrentInstance().getExternalContext().redirect("/indproc/login"); 
 	}
